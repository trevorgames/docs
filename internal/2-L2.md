...
注意：direnv 安装后一定要做 hook 到 shell 的操作，参考：https://direnv.net/docs/hook.html
...
```
注意：官方 tutorial 中 `optimism` 仓库选择的分支是 `tutorials/chain`，但其内容有些陈旧，特别是合约代码。这里我们选择撰文时最新的发行版本 `v1.7.4`，实际部署时可选择当时最新版本。
...
OP_PROPOSER_L2OO_ADDRESS=$(cat /nvme/optimism/packages/contracts-bedrock/deployments/trevor-sepolia/L2OutputOracleProxy.json | jq -r .address)
...
- [Alchemy](https://www.alchemy.com/faucets/ethereum-sepolia) 是最大方的水龙头，每 24小时能领取多个测试网上的 0.5 ETH，且 L2 上的币可以通过桥转移至 L1 ，相当于每天能领 2.5 ETH！（不过 OP Sepolia -> Seplia 桥的时效非常不友好）
...
cp scripts/getting-started/config.sh scripts/getting-started/trevor-sepolia-config.sh
vi scripts/getting-started/trevor-sepolia-config.sh
```
部分参数解释如下：
``` bash
...
echo "$config" > deploy-config/trevor-sepolia.json
```

修改好脚本之后，就可以运行它生成 L2 的配置（生成的配置文件位于`deploy-config/trevor-sepolia.json`）。
```bash
./scripts/getting-started/trevor-sepolia-config.sh
```
...
可将随机数保存下来，这样换 L1 之后只要使用相同的随机数，相关合约的地址也是相同的。而另一方面，如果要在同样的 L1 上部署一条新的链，则必须使用新的随机数。

```bash
echo $IMPL_SALT > trevor_impl_salt.txt

# 重新使用保存的随机数
export IMPL_SALT=`cat trevor_impl_salt.txt`
```

### 部署合约

接下来挑一个 L1 gas 费较低的时间窗口，运行部署合约的脚本（会部署大约46个合约！）：
```bash
